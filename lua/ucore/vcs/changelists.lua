...
function M.detail(root, provider, change_num)
  ...
  local file_rows = {}
  local lines = {
    "UCore Changelist " .. tostring(detail.number),
    string.rep("─", 60),
    "",
    "Description",
    "  " .. detail.description,
  ...
  vim.keymap.set("n", "s", function()
    local confirm = vim.fn.confirm(
      "UCore: submit changelist " .. tostring(detail.number) .. "?", "&Submit\n&Cancel", 2, "Question"
    )
  ...
  vim.keymap.set("n", "d", function()
    ...
    if not f or idx < 1 or idx > #state.files then
      vim.notify("UCore: move cursor to a file line", vim.log.levels.INFO)
      return
    end
    local diff_text, diff_err = provider.diff(f.path)
    if diff_err then return vim.notify("UCore: " .. tostring(diff_err), vim.log.levels.ERROR) end
    if not diff_text or diff_text == "" then
      return vim.notify("UCore: no diff for " .. f.name, vim.log.levels.INFO)
    end
    ...
    vim.api.nvim_buf_set_name(dbuf, "ucore://vcs/diff/" .. f.name)
  ...
  vim.keymap.set("n", "r", function()
    ...
    local confirm = vim.fn.confirm("UCore: revert " .. f.name .. "?", "&Revert\n&Cancel", 2, "Question")
    if confirm ~= 1 then return end
    provider.do_revert(f.path)
    vim.notify("UCore: reverted " .. f.name, vim.log.levels.INFO)
  end, opts)
...
function M.create_detail_buffer(lines, change_num)
  ...
  vim.api.nvim_buf_set_name(buf, "ucore://changelist/" .. tostring(change_num))

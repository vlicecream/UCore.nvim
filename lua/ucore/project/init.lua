...
-- Build default database paths under the Unreal project.
-- 在 Unreal 工程目录下构造默认数据库路径。
function M.build_paths(project_root)
	local db_dir = project_root .. "/" .. config.values.db_dir_name
	vim.fn.mkdir(db_dir, "p")

	return {
		project_root = project_root,
		db_path = db_dir .. "/ucore.db",
		cache_db_path = db_dir .. "/ucore-cache.db",
	}
end
